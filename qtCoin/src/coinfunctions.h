    ...
    if (ui->givecoinsid->text().toLatin1()!= "" ){  // user is specified
        ...
        if (ui->givecoinsid2->text() == ""){  // if
            ...
            if (admin){
                ...
                generateRXfile(result,result2,ui->givecoinsammount->text().toLatin1());
            ...
            QString result = validateID(ui->givecoinsid->text().toLatin1()).toLatin1(); // result will be unencrypted id
            qDebug() << "validate user1" << result;

            QString result2 = validateID(ui->givecoinsid2->text().toLatin1()).toLatin1(); // result will be unencrypted id
            qDebug() << "validate user2" << result2;
            float balance = checkBalanceCoins(result.toLatin1());
            qDebug() << "balance sender:" << balance;
            ...
            if(admin){
                ...
                if (ui->givecoinsammount->text().toFloat() <= balance){
                    ...
                    QMessageBox::information(nullptr, "Insufficient funds", "There is not enough balance to fullfill this transaction");
...
{// place into client walled based on yearly tables when created ID
    ...
    if (euserid.toLatin1() == "receive"){ //receiving coins back into rcoins or updating userid in recoins.db
        ...
        //db.setDatabaseName("./db/2023.sqlite");
...
float MainWindow::checkBalance(QString euserID){
    ...
    db.setDatabaseName("./db/"+euserID.toLatin1()+".sqlite");  //search all years or ask server for recoins.db(user/coins/list)
    db.open();
    QSqlDatabase::database().transaction();
    QSqlQuery query;
...
float MainWindow::checkBalanceCoins(QString euserID){
    //iterate and count coins from userid from yeardb
    float balance=0;
    qDebug() << euserID.toLatin1();
    qDebug() << "balance"+euserID.toLatin1();
    //db.setDatabaseName("./db/recoins.sqlite");
    db.setDatabaseName("./db/"+euserID.toLatin1()+".sqlite");  //search all years or ask server for recoins.db(user/coins/list)
    db.open();
    QSqlDatabase::database().transaction();
