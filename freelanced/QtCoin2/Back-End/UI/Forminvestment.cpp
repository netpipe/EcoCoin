...
void FormInvestment::InitUI()
{
    ...
    ui->txt_smtp_host->setText(m_Engine->m_SmtpInfo.strHost);
    ui->txt_smtp_email->setText(m_Engine->m_SmtpInfo.strEmail);
    ui->txt_smtp_password->setText(m_Engine->m_SmtpInfo.strPass);
    ui->txt_smtp_port->setText(QString("%1").arg(m_Engine->m_SmtpInfo.nPort));
    ui->chk_smtp_ssl->setChecked(m_Engine->m_SmtpInfo.bEncrypted);
}
...
void FormInvestment::on_btn_SmtpSave_clicked()
{
#ifdef SMTP

    QString strHost = ui->txt_smtp_host->text();
    int nPort = ui->txt_smtp_port->text().toInt();
    QString strEmail = ui->txt_smtp_email->text();
    QString strPass = ui->txt_smtp_password->text();
    bool bEncrypted = ui->chk_smtp_ssl->isChecked();
    m_Engine->m_SmtpInfo.strHost = strHost;
    m_Engine->m_SmtpInfo.nPort = nPort;
    m_Engine->m_SmtpInfo.strEmail = strEmail;
    m_Engine->m_SmtpInfo.strPass = strPass;
    m_Engine->m_SmtpInfo.bEncrypted = bEncrypted;

    m_Engine->saveSMTPInfo();
#endif
}
